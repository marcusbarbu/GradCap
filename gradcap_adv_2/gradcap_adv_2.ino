...
void pyrSwitch(uint32_t color1, uint32_t color2){
  for (int i; i<strip.numPixels();i++){
    strip.setPixelColor(i,color1);
  }
  delay(100);
  
  for (int j; j<strip.numPixels()/2;j++){
    strip.setPixelColor(j,color2);
    strip.setPixelColor(23-j,color2);
    delay(100);
  }
...
void activate(int ind){
  ...
  switch(ind){
    ...
    case1:
    alternatingColorStatic(gold, crimson);
    break; 
    
    case 2:
    pyrSwitch(crimson, gold);
    break;
    
    case 3:
    pyrSwitch(gold, crimson);
    break;
